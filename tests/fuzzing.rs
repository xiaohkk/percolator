    ...
    fn execute(&mut self, action: &Action, step: usize) {
        ...
        match action {
            ...
            Action::Withdraw { who, amount } => {
                ...
                let result = self.engine.withdraw_not_atomic(idx, *amount, oracle, now_slot, 0i128, 0, 0);
            ...
            Action::ExecuteTrade {
                ...
                let result =
                    self.engine
                        .execute_trade_not_atomic(lp_idx, user_idx, *oracle_price, now_slot, *size, *oracle_price, 0i128, 0, 0);
...
proptest! {
    ...
    fn fuzz_withdraw_decreases_or_fails(
        ...
        let result = engine.withdraw_not_atomic(user_idx, withdraw_amount, DEFAULT_ORACLE, 0, 0i128, 0, 0);
    ...
    fn fuzz_conservation_after_operations(
        ...
        for amount in withdrawals {
            let _ = engine.withdraw_not_atomic(user_idx, amount, DEFAULT_ORACLE, 0, 0i128, 0, 0);
        }
...
fn conservation_after_trade_and_funding_regression() {
    ...
    engine
        .execute_trade_not_atomic(lp_idx, user_idx, DEFAULT_ORACLE, 0, 1000, DEFAULT_ORACLE, 0i128, 0, 0)
        .unwrap();
...
fn harness_rollback_simulation_test() {
    ...
    let result = engine.withdraw_not_atomic(user_idx, 999_999, DEFAULT_ORACLE, slot, 0i128, 0, 0);
    assert!(
        result.is_err(),
